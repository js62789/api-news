  ...
  return fetchArticles(feed)
    ...
    .then(articles => {
      if (options && options.after) {
        const afterArticle = articles.find(a => a.guid === options.after);
        if (afterArticle) {
          const index = articles.indexOf(afterArticle);
          return articles.splice(0, index);
        }
      }
      return articles;
    })
...
const readAndFetchArticle = url => {
  return attachMetadata({ link: url })
    .then(article => {
      return readArticle(url)
        .then((readable) => Object.assign(article, { guid: url, content: readable.content }));
    });
};
